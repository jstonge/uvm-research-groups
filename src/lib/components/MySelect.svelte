<script lang="ts">
  ...
  // Transform simple list to object format
  ...
  const selectedLabel = $derived(
    value 
      ? optionOptions.find((option) => option.value === value)?.label
      : "Select a option"
  );
</script>

<Select.Root type="single" onValueChange={(v) => (value = v)} items={optionOptions}>
  <Select.Trigger
  ...
  <Select.Portal>
    ...
    >
      ...
      <Select.Viewport class="p-1">
        {#each optionOptions as option, i (i + option.value)}
          <Select.Item
